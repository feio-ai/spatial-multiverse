...
trait IOModule extends Module {
  ...
  val io_numArgIOs_reg = 1
  val io_argOutLoopbacksMap: scala.collection.immutable.Map[Int,Int] = Map()
  // Memory Streams
  val io_loadStreamInfo = List(StreamParInfo(32, 1, 0), StreamParInfo(32, 16, 0), StreamParInfo(32, 1, 0)) 
  val io_storeStreamInfo = List(StreamParInfo(32, 8, 0)) 
  val io_gatherStreamInfo = List(StreamParInfo(32, 1, 0)) 
  val io_scatterStreamInfo = List(StreamParInfo(32, 1, 0)) 
  val io_numArgIns_mem = 6
  ...
  // Root controller for app: HelloSpatial
  
  // Widths: 2, 2, 2, 2, 2, 3, 7
  //   Widest Outer Controller: 7
  // Depths: 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3
  //   Deepest Inner Controller: 3
  // App Characteristics: HasTileLoad,HasGather,HasScatter,HasAlignedLoad,HasUnalignedStore,HasTileStore
  // Instrumentation
