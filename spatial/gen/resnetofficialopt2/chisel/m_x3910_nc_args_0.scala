...
class x3910_nc_args_0 {
  val w0 = AccessHelper.singular(32)
  val r0 = Access(3959, 0, 0, List(0), List(0), None, PortInfo(Some(0), 1, 7, List(7), 32, List(List(RG(1,0,0)))))
  val r1 = Access(9513, 0, 1, List(1), List(0), None, PortInfo(Some(0), 1, 7, List(7), 32, List(List(RG(1,0,0)))))
  val m = Module(new LUT( 
    ...
    0,
    2, 
    myName = "x3910_nc_args_0"
  ))
  m.io.asInstanceOf[StandardInterface] <> DontCare
  m.io.reset := false.B
  ModuleParams.addParams("x3910_nc_args_0_p", m.io.p)
}
