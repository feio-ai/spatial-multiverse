...
/** Hierarchy: x9460 -> x9461 -> x9462 -> x9464 -> x18373 -> x18374 -> x1256 **/
/** BEGIN None x9460_outr_UnitPipe **/
class x9460_outr_UnitPipe_kernel(
  ...
  val me = this
  val sm = Module(new OuterControl(Streaming, 1, isFSM = false   , latency = 0.0.toInt, myName = "x9460_outr_UnitPipe_sm")); sm.io <> DontCare
  val iiCtr = Module(new IICounter(1.0.toInt, 2 + fringe.utils.log2Up(1.0.toInt), "x9460_outr_UnitPipe_iiCtr"))
  
  abstract class x9460_outr_UnitPipe_module(depth: Int)(implicit stack: List[KernelHash]) extends Module {
  ...
  def connectWires0(module: x9460_outr_UnitPipe_module)(implicit stack: List[KernelHash]): Unit = {
  ...
  def kernel(): Unit = {
    Ledger.enter(this.hashCode, "x9460_outr_UnitPipe")
    implicit val stack = ControllerStack.stack.toList
    class x9460_outr_UnitPipe_concrete(depth: Int)(implicit stack: List[KernelHash]) extends x9460_outr_UnitPipe_module(depth) {
      io.sigsOut := DontCare
      val breakpoints = io.in_breakpoints; breakpoints := DontCare
      val rr = io.rr
      val x9459_outr_UnitPipe = new x9459_outr_UnitPipe_kernel(List(x9401), List(x9388_reg,x9389_reg,x9350_out_SRAM_bias_0), List(x9399), List(x3945_reg), List(x9400), List(x2682_tmp_DRAM) , Some(me), List(), 0, 2, 1, List(1), List(32), breakpoints, rr)
      x9459_outr_UnitPipe.sm.io.ctrDone := risingEdge(x9459_outr_UnitPipe.sm.io.ctrInc)
      x9459_outr_UnitPipe.backpressure := true.B | x9459_outr_UnitPipe.sm.io.doneLatch
      x9459_outr_UnitPipe.forwardpressure := true.B | x9459_outr_UnitPipe.sm.io.doneLatch
      x9459_outr_UnitPipe.sm.io.enableOut.zip(x9459_outr_UnitPipe.smEnableOuts).foreach{case (l,r) => r := l}
      x9459_outr_UnitPipe.sm.io.break := false.B
      x9459_outr_UnitPipe.mask := true.B & true.B
      x9459_outr_UnitPipe.configure("x9459_outr_UnitPipe", Some(io.sigsIn), Some(io.sigsOut), isSwitchCase = false)
      x9459_outr_UnitPipe.kernel()
      x9350_out_SRAM_bias_0.connectStageCtrl((io.sigsIn.done).DS(1.toInt, rr, io.sigsIn.backpressure), io.sigsIn.baseEn, 2)
      x9388_reg.connectStageCtrl((io.sigsIn.done).DS(1.toInt, rr, io.sigsIn.backpressure), io.sigsIn.baseEn, 1)
      x9389_reg.connectStageCtrl((io.sigsIn.done).DS(1.toInt, rr, io.sigsIn.backpressure), io.sigsIn.baseEn, 1)
    }
    val module = Module(new x9460_outr_UnitPipe_concrete(sm.p.depth)); module.io := DontCare
...
/** END UnitPipe x9460_outr_UnitPipe **/
